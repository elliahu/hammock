...
namespace Hammock {
    ...
    */
    ...
    public:
        ...
        */
        static std::unique_ptr<Framebuffer> createFrameBufferPtrWithSharedAttachments(FramebufferCreateInfo createInfo, const std::vector<std::reference_wrapper<FramebufferAttachment>>& sharedAttachments) {
            auto fb = std::make_unique<Framebuffer>(createInfo.device);
            fb->width = createInfo.width;
            fb->height = createInfo.height;
            checkResult(fb->createSampler(createInfo.sampler.magFilter, createInfo.sampler.minFilter,
