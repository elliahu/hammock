...
namespace hammock::core {
    ...
    class DynamicCastHelper {
       public:
        virtual ~DynamicCastHelper() = default;  // makes the class polymorphic

        /// @brief Cast this object to type T*
        template <typename T>
        T* getAs();
    ...
    inline std::uint32_t getNumberOfMipLevels(const std::uint32_t width, const std::uint32_t height) {
