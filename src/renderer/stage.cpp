...
const uint8_t* hammock::renderer::GltfLoader::getAccessorData(const cgltf_accessor* accessor) {
...
void hammock::renderer::GltfLoader::loadVertices(const cgltf_primitive& prim,
    std::vector<Vertex>& vertexBuffer, uint32_t& outVertexOffset, uint32_t& outVertexCount) {
    ...
    if (posAcc->component_type != cgltf_component_type_r_32f || posAcc->type != cgltf_type_vec3) {
        throw std::runtime_error("POSITION accessor is not float3");
    }
    ...
    for (uint32_t i = 0; i < outVertexCount; ++i) {
        ...
        v.normal =
            nrm ? math::Vec3{nrm[i * 3 + 0], nrm[i * 3 + 1], nrm[i * 3 + 2]} : math::Vec3{0.f, 1.f, 0.f};
        v.uv = uv ? math::Vec2{uv[i * 2 + 0], uv[i * 2 + 1]} : math::Vec2{0.f, 0.f};
        v.tangent = {tng[i * 4 + 0], tng[i * 4 + 1], tng[i * 4 + 2], tng[i * 4 + 3]};
    }
}
void hammock::renderer::GltfLoader::visitNode(
    cgltf_node* gltfNode, Stage& stage, Scene& scene, std::int32_t parentIdx) {
    // Create the node
    SceneNode node{};
    ...
    if (gltfNode->camera) {
        ...
        node.camera = stage.cameras.size() - 1;
        ...
        if (gltfLight->type == cgltf_light_type_point) {
            ...
            core::Logger::warn("%s", "Skiped a light during loading: unsupported type");
        }

        // Set the node
        stage.lights.push_back(light);
        node.light = stage.lights.size() - 1;
    } else {
        // Node is a mesh
        if (!gltfNode->mesh) {
        ...
        for (cgltf_size i = 0; i < gltfMesh->primitives_count; i++) {
            ...
            if (gltfPrimitive.indices) {
                ...
                for (uint32_t i = 0; i < vertexCount; ++i) stage.indexBuffer.push_back(vertexOffset + i);
            }

            stage.primitives.push_back(primitive);
        }
        stage.meshes.push_back(mesh);
        node.mesh = stage.meshes.size() - 1;
    }
    ...
    if (gltfNode->has_matrix) {
        core::Logger::warn("%s", "node transform described by mat4, which is currently unsupported. skipping.");
    } else if (gltfNode->has_translation) {
        transform.position = {gltfNode->translation[0], gltfNode->translation[1], gltfNode->translation[2]};
    } else if (gltfNode->has_rotation) {
        transform.rotation = {
            gltfNode->rotation[0], gltfNode->rotation[1], gltfNode->rotation[2], gltfNode->rotation[3]};
...
void hammock::renderer::GltfLoader::parse(Stage& stage) {
